...
| Apps/connectors               | `app/list`, `app/installed`, `app/read`, `config/read`, targeted `config/value/write`, atomic `config/batchWrite`, list updates, `$app` mentions and typed `app://` context | Complete for discovery, hosted connection handoff, global policy and tool configuration | Settings separates the connected inventory from a searchable, bounded catalog of accessible and available Apps. Detail views use public `app/read` tool summaries; connection opens only the App Server-provided HTTP(S) `installUrl`, then offers an explicit authoritative refresh because no `app/install` request exists. Quick activation, shared defaults and per-App/tool policy remain App Server-owned. No unsupported persistent per-thread override, installation mutation, or client-owned App state is simulated. |
