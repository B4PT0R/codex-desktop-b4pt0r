...
| Thread creation and hydration | `thread/start`, `thread/resume`, `thread/read`, `thread/list`, `thread/turns/list`                                    | Complete for the desktop workflow                         | Recent history, active partial turns and older-page loading are covered. Multiple threads may remain active in App Server: switching changes the visible event scope immediately, then `thread/resume` restores background work and the active turn before buffered notifications continue. A configured default outside the recent page is resolved with read-only `thread/read`. `thread/turns/list` is experimental. Raw unloaded item APIs are not needed. |
...
| MCP                           | `mcpServerStatus/list`, OAuth login, `config/mcpServer/reload` and startup-status notifications                | Complete for inventory/authentication/startup visibility  | Startup state is attributed only to the current thread and cleared when that thread changes; failed reauthentication exposes the existing OAuth recovery. Users can explicitly reload `config.toml` MCP configuration before refreshing inventory. Manual generic resource/tool invocation is intentionally absent.                                                                                                              |
| Apps/connectors               | `app/list`, targeted `config/value/write`, list updates, `$app` mentions and typed `app://` context            | Complete for global activation                            | Settings exposes App Server's effective `isEnabled` state and writes only the documented `apps."<id>".enabled` key before refreshing the authoritative list. The composer receives only accessible enabled Apps. `app/installed` is reserved for a future effective-thread/callable view; no unsupported per-thread mutation is simulated.                                                                                         |
