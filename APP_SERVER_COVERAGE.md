...
This inventory was refreshed on 2026-08-09 against:

- installed `codex-cli 0.147.0`;
- its generated stable and experimental v2 JSON schemas;
- official Codex tag `rust-v0.147.0` (`3ed6f04f6`);
...
Codex 0.147 replaces the former thread `isPinned` metadata with stable,
server-owned thread sections. The client maps the built-in Pinned section back
to its existing UI concept and moves threads through `thread/section/move`.
Other 0.147 changes observed on the current surface are additive and remain
forward-compatible at the protocol boundary.
...
| Thread management             | rename, pin, fork, archive, unarchive, delete, compact                                                         | Complete                                                  | Pinning uses the stable built-in Pinned section through `thread/section/move`; the client confirms the resulting section with `thread/read`. Pinned conversations remain visible above project groups in the fully paginated catalog. Archive, unarchive and close notifications reconcile the sidebar and active session when another client changes the thread. |
...
| Installed plugins             | `plugin/installed`, targeted `config/value/write`                                                              | Complete for installed inventory and enablement            | Settings shows App Server's effective installed plugins, marketplace provenance, versions, load errors and authoritative unavailability reasons (administrator, ineligible plan or missing required App). Enablement follows the official CLI path by upserting `plugins.<id>` through App Server, then refreshing dependent Skills and MCP inventory. Catalog browsing, details, install and uninstall remain deferred while their methods are explicitly development-only. |
...
| Realtime voice v3             | start/stop, voice list, SDP, transcript, audio/session lifecycle notifications, BEM handoff routing, `thread/inject_items`, `config/read`, fork `instructionSources`, startup `initialItems` | Complete for visible and model-context persistence | Experimental voice transport with stable raw-item injection. The client uses browser-owned WebRTC only; it does **not** claim a WebSocket fallback. Codex 0.147 owns the backing model's default entry/exit mode instructions, while BEM routing preserves commentary versus final phases for the realtime intermediary. Effective `developer_instructions` from the fork workspace and AGENTS.md sources attested by its response are composed in normal precedence order through a bounded native boundary, then seeded as one developer item because Codex's generated Realtime startup context intentionally excludes them. Finalized utterances and delegated Text Agent messages are injected into the persistent parent rollout in order, including headless sessions started from the tray against the client-owned default conversation. Client-owned `msg_rtv_` and `msg_rtt_` IDs restore their distinct voice and text presentation on replay. Voice segments close once per delegated turn, while that turn's tools remain grouped inside its expandable Text Agent item. |
...
- **Threads and turns (24):** `thread/start`, `thread/resume`,
  ...
  `thread/compact/start`, `thread/section/move`, `thread/settings/update`, `thread/inject_items`,
  `thread/shellCommand`,
...
| `threadSection/list`, custom section creation/update/delete, `thread/list.sectionId` | The built-in Pinned section is mapped to the existing pin workflow. General section organization remains deferred until it has a deliberate product design. |
| App Server daemon/socket hosting                                                                    | The Unix daemon remains experimental in 0.147. Scheduler, transport and execution boundaries are separate so ownership can migrate later, but the production client keeps the proven child-process lifecycle for now.                  |
| Automation CRUD/run                                                                                 | No public App Server automation surface exists in 0.147. The native scheduler deliberately composes ordinary thread and turn methods; the chat-facing control uses the generic experimental dynamic-tool contract rather than guessing a future automation API. App-owned starts are serialized, but 0.147 has no public atomic start-if-idle primitive for excluding a simultaneous external client. |
