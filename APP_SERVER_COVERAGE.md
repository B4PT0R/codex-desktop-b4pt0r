...
| Thread creation and hydration | `thread/start`, `thread/resume`, `thread/read`, `thread/list`, `thread/turns/list`, `config/read`                     | Complete for the desktop workflow                         | Recent history, active partial turns and older-page loading are covered. At start and resume, the client reads the effective configured developer instructions for the target cwd, appends a bounded Codex Desktop identity, and supplies the result as the session's `developerInstructions`; App Server continues to discover and apply hierarchical AGENTS.md instructions separately. This avoids recording repeated per-turn application context in rollout history and lets another client resume with its own session instructions. Multiple threads may remain active in App Server: switching changes the visible event scope immediately, then `thread/resume` restores background work and the active turn before buffered notifications continue. A configured default outside the recent page is resolved with read-only `thread/read`. `thread/turns/list` is experimental. Raw unloaded item APIs are not needed. |
...
| `thread/metadata/update`                                                                            | In 0.146 this patches stored Git metadata and pin state. Pinning is additive but remains outside the frozen product surface; the client safely ignores the field and does not emit the request.                                         |
