...
if [[ $# -ne 2 || ! -f $1 || ! -f $2 ]]; then
  echo "Usage: $0 /path/to/codex-desktop-linux.deb /path/to/codex-desktop-linux.AppImage" >&2
  exit 2
fi
...
if [[ -z $container_runtime ]]; then
  if command -v podman >/dev/null 2>&1; then
    ...
    echo "Debian smoke test requires Podman or Docker." >&2
    exit 2
  fi
fi
...
"$container_runtime" run --rm \
  ...
  --volume "$deb_path:/tmp/codex-desktop.deb:ro" \
  ...
  bash -euo pipefail -c '
    ...
    if ! DEBIAN_FRONTEND=noninteractive apt-get install --yes \
      /tmp/codex-desktop.deb xvfb dbus-daemon \
    ...
    smoke_launch "Debian application" /tmp/deb-app.log \
    ...
    echo "Debian package install and DEB/AppImage headless launch smoke tests passed."
  '
