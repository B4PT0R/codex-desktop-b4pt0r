...
official `codex app-server`. The current public release is v0.5.2.
...
Release v0.5.2 packages the bounded Remote/App Server lifecycle hardening,
persisted chat-presentation controls and the final Settings primitive
normalization. It also includes the v0.5.1 authoritative App Server
`userMessage` synchronization for local, remote and steered prompts, and
preserves the v0.5.0 multi-distribution packaging baseline.
...
Agent collaboration-mode and permission-profile catalogs now invalidate both
parallel reads when their Settings surface deactivates or changes context.
